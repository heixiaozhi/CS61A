...
def accuracy(typed, source):
    ...
    for i in range(min(typed_length, source_length)):
        if typed_words[i] == source_words[i]:
            total += 1
    return total / typed_length * 100.0
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    return typed_length * (60 / elapsed) / 5
    # END PROBLEM 4
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        key = (typed, source)  # 将limit包含在缓存键中
        if key in cache:
            cached_result, cached_limit = cache[key]
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    similar_num = limit + 1
    # 求出根据diff_function对比差异最小的单词
    for word in word_list:
        # diff差异数量
        diff_num = diff_function(typed_word, word, limit)
        if diff_num <= limit and diff_num < similar_num:
            similar_num, similar_word = diff_num, word
    return similar_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    min_length = min(typed_length, source_length)
    length_difference = abs(typed_length - source_length)

    # diff 递归求差异
    def diff_length(typed_slice, source_slice, current_diff):
        # 对比结束或超过对比值
        if (current_diff > limit) or len(typed_slice) == 0:
            return current_diff
        # 对比
        num = 0
        if typed_slice[0] != source_slice[0]:
            num = 1
        # 下一个对比和累计结果
        return diff_length(typed_slice[1:], source_slice[1:], current_diff + num)

    return diff_length(typed[:min_length], source[:min_length], length_difference)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    for i, x in enumerate(typed):
        if x != source[i]:
            break
        right_num += 1
    play1_progress = right_num / len(source)
    # 上传
    upload({"id": user_id, "progress": play1_progress})
    print(play1_progress)
    # END PROBLEM 8
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        ...
        for player in player_indices:
            player_times = get_time(times, player, word_index)
            if player_times < min_time:
                min_time = player_times
                min_player = player
        # 将单词加入到对应的玩家之中
        ans[min_player].append(words[word_index])
    return ans
    # END PROBLEM 10
