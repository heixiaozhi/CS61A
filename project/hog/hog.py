...
def boar_brawl(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 2
    current_score = player_score % 10
    other_score = opponent_score // 10 % 10
    score = max(abs(other_score - current_score) * 3, 1)
    return score
...
def make_averaged(original_function, times_called=1000):
    ...
    # *args 获取所有参数
    def make_averaged_dice(*args):
        # sum_score, k = 0, 0
        # while k < times_called:
        #     # roll_dice(1, make_test_dice(4, 2, 5, 1))
        #     sum_score += original_function(*args)
        #     k += 1
        sum_score = sum([original_function(*args) for i in range(times_called)])
        return sum_score / times_called

    return make_averaged_dice
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_averaged = 0
    for i in range(1, 11):
        current_averaged = averaged_dice(i, dice)
        if current_averaged > max_averaged:
            max_num_rolls, max_averaged = (
                i,
                current_averaged,
            )
